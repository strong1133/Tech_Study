...
- 이렇게 객체에서 다른 객체를 부를때 직접 객체를 생성하지 않고 이미 생성되어 있는 객체를 가져오는 작업을 `의존성 주입` 이라 하며 영어로 `DI - Dependency Injection` 라고 한다.
        ...
        만약 A가 B를 필요로 할때 A 가 B를 만드는 A->B 가 아닌 B에게 너가 필요하니까 와줄수 있니? 하며 요청하는 A <- B 가 되므로 제어의 흐름이 역전됬다 하는 의미로 '제어의 역전 (IoC: Inversion of Control)' 라고 부르게 된다.
